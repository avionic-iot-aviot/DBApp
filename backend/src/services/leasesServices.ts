...
export default class LeasesServices {
    ...
    async InsertDevice(leases: ILeases) {
        let temp: IDevice;
        temp = { device_id: null, mac_address: leases.mac, default_name: leases.host, current_name: `${leases.host}-${leases.mac.replace(/:/g, "").toLowerCase()}`, is_static: leases.isStatic, is_device: leases.isADevice, is_drone: leases.isADevice, ip: leases.ip, is_active: leases.isActive, copter_id: leases.copterID };
